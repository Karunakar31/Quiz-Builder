...
import styles from './QuizModalPage2.module.css';
...
const QuizModalPage2 = (props) => {
  ...
  const saveChanges = () => {
    ...
    if (!question && !delQuestion) {
      toastMessage("Please enter a question", "warning");
      return;
    }

    if (quizInfo.type ==="qna" && selectedOption === -1 && !delQuestion) {
    ...
    if (optionRef[0].current) {
      if (!optionRef[0].current.value && !delQuestion) {
    ...
    if (optionRef[2].current) {
      if (!optionRef[2].current.value && !delQuestion) {
        toastMessage("Please enter option 3", "warning");
        return;
      }
      option3 = optionRef[2].current.value;
        if(option3 === option1 || option3 === option2 && !delQuestion){
            toastMessage("Please enter different options", "warning");
            return;
        }
    }
    ...
    if (optionRef[3].current) {
      if (!optionRef[3].current.value && !delQuestion) {
        toastMessage("Please enter option 4", "warning");
        return;
      }
      option4 = optionRef[3].current.value;
        if(option4 === option1 || option4 === option2 || option4 === option3 && !delQuestion){
            toastMessage("Please enter different options", "warning");
            return;
        }
    }
    ...
    if (optionImgRef[0].current) {
      if (!optionImgRef[0].current.value && !delQuestion) {
    ...
    if (optionImgRef[2].current) {
      if (!optionImgRef[2].current.value && !delQuestion) {
        toastMessage("Please enter option 3 image URL", "warning");
        return;
      }
      option3img = optionImgRef[2].current.value;
        if(option3img === option1img || option3img === option2img && !delQuestion){
            toastMessage("Please enter different options", "warning");
            return;
        }
    }
    ...
    if (optionImgRef[3].current) {
      if (!optionImgRef[3].current.value && !delQuestion) {
        toastMessage("Please enter option 4 image URL", "warning");
        return;
      }
      option4img = optionImgRef[3].current.value;
        if(option4img === option1img || option4img === option2img || option4img === option3img && !delQuestion){
            toastMessage("Please enter different options", "warning");
            return;
        }
    }
  ...
  const populate = async (index) => {
    ...
    if (questions[index].timer === 0) {
      setSelectedTimer("0");
    } else if (questions[index].timer === 5) {
      setSelectedTimer("5");
    } else {
      setSelectedTimer("10");
    }
    ...
    <div className={styles.page2}>
      <div className={styles.topbar}>
        <div className={styles.topbarleft}>
          ...
          {questionNumber.length < 5 && (
            <div onClick={handleAddQuestion} className={styles.questionselector}>
              <FiPlus />
            </div>
          )}
      ...
      <div className={styles.bottombar}>
        <div className={styles.bottomleft}>
          ...
          {options.length < 4 && (
            <div onClick={handleAddOption} className={quizInfo.type!=="poll" ? styles.addoption: styles.polladdoption}>
              Add Option
            </div>
          )}
        </div>
        <div className={styles.bottomright}>
          {quizInfo.type!=="poll"&&<div className={styles.timeroptions}>
            <h6>Timer</h6>
            <div
      ...
      <div className={styles.cancelconfirm}>
        ...
        <button onClick={handleCreateQuiz} className={styles.confirmbtn}>Create Quiz</button>
      </div>
    </div>
  );
};
